...
var cityGroup = {
    ...
    "Large-Faster Growth":"#e3587b",
    "Rust Belt-Declining":"#61d89a",
    "New Economy-Quality of Life Hubs":"#f5955e",
...
function drawKey(keyData){
  //  console.log("key")
    ...
    var keySvg=d3.select("#bubble-key").append("svg").attr("width",220).attr("height",height)
    ...
    .append("text")
    .attr("x",50)
    .attr("y",function(d,i){return i*size+20})
    .text(function(d){
      ...
      return d.key})
    .attr("fill",function(d){return cityGroup[d.key]})
...
function generateAxis(list,data,xSelection,ySelection,rLabel){
    ...
    d3.select("#centered").append("select").attr("id","yAxisData")
          d3.select("#chartTitle").html("Comparison of MSAs: "+ySelection.split("_").join(" ")+"/"+xSelection.split("_").join(" "))

    d3.select("#centered").append("div").attr("id","xAxisArrow").append("svg").attr("width",10).attr("height",10)
    .append("svg:image")
    .attr("x",0)
    .attr("y",0)
    .attr("width",8)
    ...
    d3.select("#centered").append("div").attr("id","yAxisArrow").append("svg").attr("width",10).attr("height",10).append("svg:image")
    .attr("x",0)
    .attr("y",0)
    .attr("width",8)
    .attr("height",8)
